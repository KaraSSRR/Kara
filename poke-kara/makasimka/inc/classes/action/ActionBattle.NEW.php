            ...
            if ($this->userTarget && $this->enemyTarget && $target > 0 && isset($this->userTarget['atkList']['a' . $target])) {
                if ($this->userTarget['hp'] > 0) {
                    ...
                    if ($disable[$atk['attack_num']] == 0 && isset($atk['id'])) {
                        $this->userData['mega']      = isset($_POST['mega']) ? 1 : 0;
                        $this->userData['targetAtk'] = $atk['id'];
...
private function goRound() {
    ...
    if ($canStart) {
        ...
        if (!empty($extraRoundLog)) {
            $logArr['log'] = array_merge($logArr['log'], $extraRoundLog);
        }
        $this->log[] = $logArr;
...
private function _getNameBattle($pok) {
    ...
    $name    = htmlspecialchars($pok['name_new'] ?? '???');
...
public function lose($user_lose_id, $lose_type = 'OTHER', $last_atk = false) {
    ...
    if (!empty($user_winner['id']) || $lose_type == 'CATCH') {
        ...
        elseif (!$this->_isPVP() && !$isWorldBoss && isset($this->battleInfo['user_2']) && (int)$this->battleInfo['user_2'] === 0) {
            ...
            $kolco = (isset($this->userTarget['item_id']) && $this->userTarget['item_id'] == 157) ? mt_rand(25, 100) : 0;
            $lvl   = (int)($this->enemyTarget['lvl'] ?? 1);

            if ($lvl == 4) {
                $money = mt_rand(60, 80);
            } elseif ($lvl >= 15 && $lvl <= 20) {
                $money = mt_rand(190, 250);
            } elseif ($lvl >= 40 && $lvl <= 50) {
                $money = mt_rand(350, 420);
            } else {
                $moneyBase     = 12 * pow(max(1, $lvl), 0.95);
                $randomFactor  = mt_rand(87, 113) / 100;
                $money         = $moneyBase * $randomFactor;
                $money        *= (float)($systemBonuses['money'] ?? 1.0);
                $money        *= $moneyBuff;
                $money        *= $bafPr;
            ...
            $maxMoney = ($bafPr > 1.0) ? 2000 : 1500;
            $money = min($money, $maxMoney);

            if (achiv_utility(1)) {
                $money = (int)round($money * 1.1);
            }
            update_achiv(1, $money);
...
public function _isPVP() {
    return (isset($this->battleType) && strtolower($this->battleType) === 'pvp');
}
