...
-- Extend user_creatures with Showdown-like fields (idempotent)
-- MySQL 5.7 does NOT support "ADD COLUMN IF NOT EXISTS", so we guard via INFORMATION_SCHEMA.
...
SET @col_exists := (
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'user_creatures'
    AND COLUMN_NAME = 'nature'
);
SET @sql := IF(
  @col_exists = 0,
  'ALTER TABLE `user_creatures` ADD COLUMN `nature` VARCHAR(16) NOT NULL DEFAULT ''hardy''',
  'SELECT 1'
);
...
SET @sql := IF(
  @col_exists = 0,
  'ALTER TABLE `user_creatures` ADD COLUMN `happiness` SMALLINT UNSIGNED NOT NULL DEFAULT 70',
  'SELECT 1'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql := IF(
  @col_exists = 0,
  'ALTER TABLE `user_creatures` ADD COLUMN `ability_id` INT UNSIGNED NULL',
  'SELECT 1'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @col_exists := (
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'user_creatures'
    AND COLUMN_NAME = 'held_item_id'
);
SET @sql := IF(
  @col_exists = 0,
  'ALTER TABLE `user_creatures` ADD COLUMN `held_item_id` INT UNSIGNED NULL',
  'SELECT 1'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql := IF(
  @col_exists = 0,
  'ALTER TABLE `user_creatures` ADD COLUMN `is_shiny` TINYINT(1) NOT NULL DEFAULT 0',
  'SELECT 1'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @idx_exists := (
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'user_creatures'
    AND INDEX_NAME = 'idx_user_creatures_ability'
);
SET @sql := IF(
  @idx_exists = 0,
  'ALTER TABLE `user_creatures` ADD KEY `idx_user_creatures_ability` (`ability_id`)',
  'SELECT 1'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @idx_exists := (
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'user_creatures'
    AND INDEX_NAME = 'idx_user_creatures_held_item'
);
SET @sql := IF(
  @idx_exists = 0,
  'ALTER TABLE `user_creatures` ADD KEY `idx_user_creatures_held_item` (`held_item_id`)',
  'SELECT 1'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Foreign keys (guard by column+reference, not by FK name)
SET @fk_exists := (
  SELECT COUNT(*)
  FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE kcu
  WHERE kcu.TABLE_SCHEMA = DATABASE()
    AND kcu.TABLE_NAME = 'user_creatures'
    AND kcu.COLUMN_NAME = 'ability_id'
    AND kcu.REFERENCED_TABLE_NAME = 'abilities'
    AND kcu.REFERENCED_COLUMN_NAME = 'id'
);
SET @sql := IF(
  @fk_exists = 0,
  'ALTER TABLE `user_creatures` \
     ADD CONSTRAINT `fk_user_creatures_ability_id` \
...
SET @fk_exists := (
  ...
  WHERE kcu.TABLE_SCHEMA = DATABASE()
    AND kcu.TABLE_NAME = 'user_creatures'
    AND kcu.COLUMN_NAME = 'held_item_id'
...
SET @sql := IF(
  @fk_exists = 0,
  'ALTER TABLE `user_creatures` \
     ADD CONSTRAINT `fk_user_creatures_held_item_id` \
