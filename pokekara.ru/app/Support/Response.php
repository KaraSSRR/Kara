...
final class Response {
    ...
    public static function apiOk(Request $request, array $data = [], int $status = 200): void {
        // SPA/PJAX requests expect a *flat* payload like {title, html, flash?} or {redirect}.
        // API requests expect an envelope {ok:true,data:{...},request_id}.
        if ($request->wantsPartial() && !str_starts_with($request->path, '/api/')) {
